...
export function usePagination({
	...
	// Ellipsis display follows the pages actually shown, not the raw totals.
	const showLeftEllipsis = pages.length > 0 && (pages[0] ?? 0) > 2;

	const showRightEllipsis =
		pages.length > 0 && (pages[pages.length - 1] ?? 0) < totalPages - 1;
